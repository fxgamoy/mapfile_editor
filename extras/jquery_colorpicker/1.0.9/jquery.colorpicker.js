				...
				this.init = function() {
					...
					if (!inst.inline && inst.options.draggable) {
						var draggableOptions = {
							handle: e,
							//fx pour capter le début du drag
							// Dans le cas de modal false,
	 						// on ajoute le div d'overlay avec une opacité de 0 
	 						// comme écran qui arrete la propagation des event  vers openlayers 
							start: function( event, ui ) {
								if(!inst.options.modal) {
									$jq(this).before("<div class='ui-widget-overlay ui-front'></div>");
									$jq('.ui-widget-overlay').css('opacity','0');
								}
							},
							stop: function( event, ui ) {
								if(!inst.options.modal) {
									if($jq('.ui-widget-overlay').length>0)
										$jq('.ui-widget-overlay').remove();
								}
							},
							// fin fix fx
						}
